...
Defaults to `false`.
`transport`:: Which transport to use if available. If the attempt to use native
transport fails, falls back to java NIO transport. This option is independent
of Vertx transport configuration and affects only the MongoDB driver's
networking. Supported values are: `nio`, `epoll`, `io_uring` and `kqueue`.
Defaults to `nio`.
