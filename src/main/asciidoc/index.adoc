...
Defaults to `false`.
`transport`:: Which transport to use if available.
Supported values are: `nio`, `epoll`, `io_uring`, `kqueue` for specific
transport, and `auto` for using the same transport as vertx is using.
If the attempt to use specified transport fails, falls back to the same
transport used by vertx (`auto` behavior).
Defaults to `auto`.
