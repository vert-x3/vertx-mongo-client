...
`sslInvalidHostNameAllowed`:: trust hostnames not matching the certificate common name.
...
`keyPath`:: Set a path to a file that contains a private key that will be used to authenticate SSL connections to mongo.
`certPath`:: Set a path to a file that contains a certificate that will be used to authenticate SSL connections to mongo.
