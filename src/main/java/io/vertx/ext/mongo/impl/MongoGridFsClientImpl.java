...
public class MongoGridFsClientImpl implements MongoGridFsClient {
  ...
  public MongoGridFsClientImpl(VertxInternal vertx, MongoClientImpl mongoClient, GridFSBucket gridFSBucket, CodecRegistry codecRegistry) {
    ...
    this.tracingPolicy = mongoClient.tracingPolicy();
  }

  private MongoTracerRequest.Builder tracingRequest(String operation) {
    return MongoTracerRequest.create(clientImpl.databaseName(), bucket.getBucketName(), "mongo.gridfs." + operation);
  }
  ...
  private <T> void subscribeWithTracing(Promise<?> promise, MongoTracerRequest.Builder builder, Publisher<? extends T> publisher, Subscriber<? super T> subscriber) {
    MongoTracer.subscribe(contextFromPromise(promise), builder.build(), tracingPolicy, publisher, subscriber);
  }
  ...
  private <T> PublisherAdapter<T> tracedAdapter(Publisher<T> publisher, int batchSize, MongoTracerRequest request) {
    ...
    return new PublisherAdapter<>(context, MongoTracer.publisher(context, request, tracingPolicy, publisher), batchSize);
  }
