...
----
var query = json {
  obj("metadata.nick_name" to "Puhi the eel")
}
...
var bufferSize = 1024
var id = "56660b074cedfd000570839c"
var stream = java.io.FileOutputStream("eel.fil")
gridFsClient.downloadBufferById(id, { res ->
  if (res.succeeded()) {
    var download = res.result()
    var downloadCompleteFuture = Future.future<Any>()
    downloadCompleteFuture.setHandler({ downloadRes ->
      if (downloadRes.succeeded()) {
        //Download finished
      } else {
        //Something went wrong
        throw java.lang.RuntimeException(downloadRes.cause())
      }
    })
